...
//
// NAMING TRAP: this file / class is Helper Form 4 on mobile, but it is
// web sidebar Helper 5 (Metab / Renal / Eye). Do not rename without a
...
bool _isEmptyGlucoseField(String? v) => v == null || v.trim().isEmpty;
...
class _HelperForm4MetabRenalVascEyeState
  ...
  MetabRenalVascEyeDay _buildModel() {
    ...
    m.lowestGlucose = _lowGlucoseCtrl.text.trim().isEmpty
        ? null
        : _lowGlucoseCtrl.text.trim();
    ...
    m.phReadings = _model.phReadings
        .map(
          (r) =>
              MrveReading(id: r.id, date: r.date, time: r.time, value: r.value),
        )
        .toList();
    ...
    m.axillaryTemperature = _axillaryCtrl.text.trim().isEmpty
        ? null
        : _axillaryCtrl.text.trim();
  ...
  Future<bool> _applyLocalDraftIfAny(
    ...
    if (!helperLocalDraftIsForScreening(
      ...
      await HelperDayDraftStorage.clear(_kMrveDraftKey, eid, day);
      return false;
    }
    _applyDay(MetabRenalVascEyeDay.fromJson(raw));
    return true;
  ...
  Future<bool> _applyGlucoseAutofill({required bool force}) async {
    ...
    try {
      ...
      void maybeSet(String key, TextEditingController ctrl) {
        ...
        final stillMatches =
            lastComputed != null && current.trim() == lastComputed!.trim();
      ...
      body: Column(
        children: [
          HelperPatientHeader(
            ...
            subtitle:
                'Daily metabolic, renal, vascular & ophthalmology assessment',
  ...
  List<Widget> _metabolicFields(AppColors c, bool editable) {
    return [
      _glucoseField(
        c,
        '1',
        'Lowest glucose reading (if <45 mg/dL)',
        _lowGlucoseCtrl,
        editable,
        'mg/dL',
        _glucoseAutofilled['lowest_glucose']!,
      ),
      _glucoseField(
        c,
        '2',
        'No of episodes of hypoglycemia',
        _hypoEpisodesCtrl,
        editable,
        null,
        _glucoseAutofilled['hypoglycemia_episodes']!,
      ),
      if (_hypoRxRequired)
      ...
      _glucoseField(
        c,
        '4',
        'Highest glucose reading (if >125 mg/dL)',
        _highGlucoseCtrl,
        editable,
        'mg/dL',
        _glucoseAutofilled['highest_glucose']!,
      ),
      if (_hyperRxRequired)
      ...
      _yn(
        '10. Osteopenia suspected',
        _model.osteopeniaSuspected,
        editable,
        (v) => setState(() => _model.osteopeniaSuspected = v),
        c,
      ),
    ];
  }
  ...
  List<Widget> _renalFields(AppColors c, bool editable) {
    return [
      ...
      _yn(
        ...
        _model.akiSuspected,
        editable,
        (v) => setState(() => _model.akiSuspected = v),
        c,
      ),
      _fieldCard(
        c,
        ...
        child: _textField(_creatinineCtrl, c, enabled: editable, hint: 'mg/dL'),
      ),
      _fieldCard(
        c,
        number: '13a',
      ...
      _yn(
        ...
        _model.dialysisCrrt,
        editable,
        (v) => setState(() => _model.dialysisCrrt = v),
        c,
      ),
    ];
  }
  ...
  List<Widget> _vascularFields(AppColors c, bool editable) {
    return [
      _yn(
        ...
        _model.piccInSitu,
        editable,
        (v) => setState(() => _model.piccInSitu = v),
        c,
      ),
      _yn(
        '17. UVC in situ',
      ...
      if (_extravasationRequired)
        _yn(
          '21. Extravasation injury',
          _model.extravasationInjury,
          editable,
          (v) => setState(() => _model.extravasationInjury = v),
          c,
        ),
      _yn(
        '22. Line complication',
        _model.lineComplication,
        editable,
        (v) => setState(() => _model.lineComplication = v),
        c,
      ),
    ];
  }
    ...
    return [
      ...
      if (_ropYes) ...[
        _yn(
          ...
          _model.plusDisease,
          editable,
          (v) => setState(() => _model.plusDisease = v),
          c,
        ),
        _yn(
          'ROP Treatment',
          _model.ropTreatment,
          editable,
          (v) => setState(() => _model.ropTreatment = v),
          c,
        ),
      ],
    ];
    ...
    return Wrap(
      ...
      children: options.map((o) {
        ...
        return FilterChip(
          label: Text(o),
          selected: sel,
          onSelected: !enabled ? null : (_) => onChanged(sel ? null : o),
        );
      }).toList(),
    );
  }
